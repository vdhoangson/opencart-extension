...
class ControllerExtensionPenBlogArticle extends Controller {
	...
	public function index() {
		$this->load->language('extension/penblog/article');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_penblog_article->addArticle($this->request->post);
			...
			$this->response->redirect($this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_extension_penblog_article->editArticle($this->request->get['article_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $article_id) {
				$this->model_extension_penblog_article->deleteArticle($article_id);
			}
	...
	public function copy(){
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $article_id) {
				$this->model_extension_penblog_article->copyArticle($article_id);
			}
	...
	protected function getList() {
		$languages = $this->load->language('extension/penblog/article');
		...
		$data['categories'] = $this->model_extension_penblog_category->getCategories($data);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('extension/penblog/article/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('extension/penblog/article/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['copy'] = $this->url->link('extension/penblog/article/copy', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['articles'][] = array(
				...
				'comment' 	=> $this->model_extension_penblog_comment->getTotalCommentsByArticleId($result['article_id']),
				'status'     => ($result['status']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'edit'       => $this->url->link('extension/penblog/article/edit', 'user_token=' . $this->session->data['user_token'] . '&article_id=' . $result['article_id'] . $url, true),
		...
		$data['sort_name'] = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=pd.name' . $url, true);
		$data['sort_date_added'] = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.date_modified' . $url, true);
		$data['sort_status'] = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('extension/penblog/article_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['article_id'])) {
			$data['action'] = $this->url->link('extension/penblog/article/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('extension/penblog/article/edit', 'user_token=' . $this->session->data['user_token'] . '&article_id=' . $this->request->get['article_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('extension/penblog/article', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['article_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$article_info = $this->model_extension_penblog_article->getArticle($this->request->get['article_id']);
		}
		...
		if (isset($this->request->post['article_description'])) {
			...
			$data['article_description'] = $this->model_extension_penblog_article->getArticleDescriptions($this->request->get['article_id']);
		} else {
		...
		if (isset($this->request->post['article_store'])) {
			...
			$data['article_store'] = $this->model_extension_penblog_article->getArticleStores($this->request->get['article_id']);
		} else {
		...
		if (isset($this->request->post['article_seo_url'])) {
			...
			$data['article_seo_url'] = $this->model_extension_penblog_article->getArticleSeoUrls($this->request->get['article_id']);
		} else {
		...
		if (isset($this->request->post['article_image'])) {
			...
			$article_images = $this->model_extension_penblog_article->getArticleImages($this->request->get['article_id']);
		} else {
		...
		if (isset($this->request->post['article_category'])) {
			...
			$categories = $this->model_extension_penblog_category->getCategoriesByArticleId($this->request->get['article_id']);
		} else {
			$categories = array();
		}
		...
		if (isset($this->request->post['article_related'])) {
			...
			$articles = $this->model_extension_penblog_article->getArticleRelated($this->request->get['article_id']);
		} else {
			$articles = array();
		}
		...
		foreach ($articles as $article_id) {
			$related_info = $this->model_extension_penblog_article->getArticle($article_id);
		...
		// Download
		$this->load->model('extension/penblog/download');

		if (isset($this->request->post['article_download'])) {
			$downloads = $this->request->post['article_download'];
		} elseif (isset($this->request->get['article_id'])) {
			$downloads = $this->model_extension_penblog_download->getDownloadByArticleId($this->request->get['article_id']);
		} else {
			$downloads = array();
		}
		...
		if (isset($this->request->post['article_product'])) {
			...
			$products = $this->model_extension_penblog_article->getProductsByArticleId($this->request->get['article_id']);
		} else {
			$products = array();
		}
		...
		$data['polls'] = $this->model_extension_penblog_poll->getPolls();

		if (isset($this->request->post['poll_id'])) {
      		$data['poll_id'] = $this->request->post['poll_id'];
		} elseif (!empty($article_info)) {
			$data['poll_id'] = $this->model_extension_penblog_poll->getPollByArticleId($article_info['article_id']);
		} else {
      		$data['poll_id'] = 0;
    	}
		...
		if (isset($this->request->post['article_layout'])) {
			...
			$data['article_layout'] = $this->model_extension_penblog_article->getArticleLayouts($this->request->get['article_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('extension/penblog/article_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/penblog/article')) {
