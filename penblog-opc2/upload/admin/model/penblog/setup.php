...
*******************************************************/
class ModelPenBlogSetup extends Model {
	...
	public function install(){
		...
		$queries[] = "CREATE TABLE `" . DB_PREFIX . "penblog_article_to_download` (
					  `article_id` int(11) NOT NULL,
					  `download_id` int(11) NOT NULL,
					  PRIMARY KEY (`article_id`,`download_id`)
					) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;";

		$queries[] = "CREATE TABLE `" . DB_PREFIX . "penblog_article_to_product` (
					  `article_id` int(11) NOT NULL,
		...
		foreach($this->getController() as $code){
			...
			if (!$this->user->hasPermission('modify', 'penblog/'. $code)) {
				$this->model_user_user_group->addPermission($this->user->getId(), 'access', 'penblog/'. $code);
				$this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'penblog/'. $code);
			}
		}
