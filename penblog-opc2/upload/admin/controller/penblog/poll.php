...
class ControllerPenBlogPoll extends Controller
{
    ...
    public function add()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_penblog_poll->addPoll($this->request->post);
            ...
            $this->response->redirect($this->url->link('penblog/poll', 'token=' . $this->session->data['token'] . $url, true));
        }
    ...
    public function edit()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_penblog_poll->editPoll($this->request->get['poll_id'], $this->request->post);
    ...
    public function delete()
    {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            foreach ($this->request->post['selected'] as $poll_id) {
                $this->model_penblog_poll->deletePoll($poll_id);
            }
    ...
    private function getList()
    {
        ...
        $poll_lang = $this->load->language('penblog/poll');
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('penblog/poll', 'token=' . $this->session->data['token'] . $url, true),
            'separator' => ' :: '
        );
        
        $data['insert'] = $this->url->link('penblog/poll/add', 'token=' . $this->session->data['token'] . $url, true);
        $data['delete'] = $this->url->link('penblog/poll/delete', 'token=' . $this->session->data['token'] . $url, true);
        ...
        $poll_total = $this->model_penblog_poll->getTotalPolls();
        
        $results = $this->model_penblog_poll->getPolls($filter_data);
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('penblog/poll/edit', 'token=' . $this->session->data['token'] . '&poll_id=' . $result['poll_id'] . $url, true)
            );
        ...
        $data['sort_question'] = $this->url->link('penblog/poll', 'token=' . $this->session->data['token'] . '&sort=question' . $url, true);
        $data['sort_status']   = $this->url->link('penblog/poll', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
        ...
        $pagination->url   = $this->url->link('penblog/poll', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
        ...
        if ((int) substr((string) str_replace('.', '', VERSION), 0, 3) > 210) {
            $this_template = 'penblog/poll_list';
        } else {
            $this_template = 'penblog/poll_list.tpl';
        }
    ...
    private function getForm()
    {
        ...
        if (isset($this->request->get['poll_id'])) {
            ...
            $reactions                 = $this->model_penblog_poll->getPollResults($poll_id);
        ...
        if (!isset($this->request->get['poll_id'])) {
            $data['new_poll'] = TRUE;
            $data['action']   = $this->url->link('penblog/poll/add', 'token=' . $this->session->data['token'], true);
        } else {
            $data['action'] = $this->url->link('penblog/poll/edit', 'token=' . $this->session->data['token'] . '&poll_id=' . $this->request->get['poll_id'], true);
        }
        $data['cancel']       = $this->url->link('penblog/poll', 'token=' . $this->session->data['token'], true);
        $data['poll_setting'] = $this->url->link('module/ave_community_poll', 'token=' . $this->session->data['token'], true);
        
        
        if ((isset($this->request->get['poll_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $poll_info = $this->model_penblog_poll->getPoll($this->request->get['poll_id']);
        }
        ...
        if (isset($this->request->post['poll_description'])) {
            ...
            $data['poll_description'] = $this->model_penblog_poll->getPollDescriptions($this->request->get['poll_id']);
        } else {
        ...
        if (isset($this->request->post['poll_store'])) {
            ...
            $data['poll_store'] = $this->model_penblog_poll->getPollStores($this->request->get['poll_id']);
        } else {
        ...
        if ((int) substr((string) str_replace('.', '', VERSION), 0, 3) > 210) {
            $this_template = 'penblog/poll_form';
        } else {
            $this_template = 'penblog/poll_form.tpl';
        }
        $this->response->setOutput($this->load->view($this_template, $data));
    }
    ...
    private function validateForm()
    {
        ...
        if (!$this->user->hasPermission('modify', 'penblog/poll')) {
