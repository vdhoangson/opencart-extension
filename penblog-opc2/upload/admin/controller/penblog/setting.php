...
*******************************************************/
class ControllerPenBlogSetting extends Controller {
	private $error = array();

	public function index() {
		$languages = $this->load->language('penblog/setting');
		...
		$this->load->model('penblog/setting');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {

			$this->model_penblog_setting->editSetting('penblog', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('penblog/setting', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('penblog/setting', 'token=' . $this->session->data['token'], true);
		...
		$data['date_formats'] = array(
			'd-m-y' => 'd-m-y (08-03-16)',
			'y-m-d' => 'y-m-d (16-03-08)',
			'd-m-Y' => 'd-m-Y (08-03-2016)',
			'Y-m-d' => 'Y-m-d (2016-03-08)',
			'd/m/y' => 'd/m/Y (08/03/16)',
			'd/m/Y' => 'd/m/Y (08/03/2016)',
			'Y/m/d' => 'Y/m/d (2016/08/03)',
		);
		...
		$this->response->setOutput($this->load->view('penblog/setting.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'penblog/setting')) {
