...
class ControllerPenBlogCategory extends Controller {
	public function index() {
		$languages = $this->load->language('penblog/category');
			...
			foreach ($parts as $path_id) {
				...
				if ($category_info) {
					$data['breadcrumbs'][] = array(
						...
						'href' => $this->url->link('penblog/category', 'penblog_path=' . $path . $url)
					);
				}
			}
		} else {
		...
		if ($category_info) {
			...
			$this->document->addLink($this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path']), 'canonical');
			...
			$data['breadcrumbs'][] = array(
				'text' => $category_info['name'],
				'href' => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'])
			);

			if ($category_info['image']) {
				$data['thumb'] = $this->model_penblog_image->cropsize($category_info['image'], $setting['catalog_image_width'], $setting['catalog_image_height']);
			} else {
				$data['thumb'] = '';
			}
			...
			$results = $this->model_penblog_category->getCategories($category_id);
			...
			foreach ($results as $result) {
				...
				$data['categories'][] = array(
					'name'  => $result['name'] . ($this->config->get('config_article_count') ? ' (' . $this->model_penblog_article->getTotalArticles($filter_data) . ')' : ''),
					'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . '_' . $result['category_id'] . $url)
				);
			}
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . '&sort=p.sort_order&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . '&sort=pd.name&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . '&sort=pd.name&order=DESC' . $url)
			);
			...
			$data['sorts'][] = array(
				...
				'value' => 'rating-DESC',
				'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . '&sort=rating&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_rating_asc'),
				'value' => 'rating-ASC',
				'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . '&sort=rating&order=ASC' . $url)
			);
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . $url . '&limit=' . $value)
				);
			}
			...
			$pagination->url = $this->url->link('penblog/category', 'penblog_path=' . $this->request->get['penblog_path'] . $url . '&page={page}');
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('penblog/category', $url)
			);
