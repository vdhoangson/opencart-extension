...
*******************************************************/
class ControllerModulePenBlogBlock extends Controller {

	public function index($setting = array()){
		$data['block'] = '';

		if(file_exists(DIR_APPLICATION . 'controller/penblog_block/' . $setting['block']. '.php')){
			$data['block'] = 	 $this->load->controller('penblog_block/'.$setting['block'], $setting);
		}

		$template = 'module/penblog_block';
	...
	public function tab_item(){
		...
		if(isset($this->request->get['category_id'])){
			...
			$results = $this->model_penblog_article->getArticlesByCategory($this->request->get['category_id'], $limit);
			...
			foreach($results as $result){
				...
				if ($result['image']) {
					...
					if($img_type['type']=='image'){
						...
						$thumb = $this->model_penblog_image->cropsize($img_type['image'], $block_data['image_width'], $block_data['image_height']);
					...
					$thumb = $this->model_penblog_image->cropsize('no_image.png', $block_data['image_width'], $block_data['image_height']);
				}
				...
				$data['categories'] = $this->model_penblog_article->getCategoriesByArticleId($result['article_id']);
		...
		$template = 'module/penblog/category_data';
		if((int)str_replace('.', '', VERSION) < 2200){
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/'.$template.'.tpl')) {
